...
from utils import (fetch_subject_list, get_hcp_fmri_fname, get_protocol_hcp,
                   TR_HCP)
...
date = datetime.now()
dirname = 'results_hcp_#{0}{1}{2}{3}{4}{5}'.format(date.year,
                                                   date.month,
                                                   date.day,
                                                   date.hour,
                                                   date.minute,
...
# True paradigm
plt.figure("True paradigm", figsize=(10, 5))
conditions = ['lh', 'rh', 'lf', 'rf']
for i, cond in enumerate(conditions):
    plt.subplot(len(conditions), 1, i+1)
    _, _, p_e, _ = get_protocol_hcp(subject_id, cond)
    plt.plot(p_e, lw=2.0)
    plt.title(cond)
    plt.grid()
plt.tight_layout()
